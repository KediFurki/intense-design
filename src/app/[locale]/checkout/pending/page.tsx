...
export default async function CheckoutPendingPage({ searchParams }: { searchParams: { oid?: string } }) {
  ...
  const order = await db.query.orders.findFirst({
    ...
    columns: { id: true, paymentMethod: true, paymentDueAt: true, totalAmount: true, remainingAmount: true, depositPercent: true }
  });
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-xl w-full bg-white border rounded-lg p-6 space-y-3">
        ...
        <p className="text-slate-600">Order ID: <b>{order?.id}</b></p>
        ...
        {order?.paymentMethod === "iban" ? (
          <>
            ...
            <div className="bg-slate-50 border rounded p-3 text-sm space-y-1">
              ...
              <div><b>Amount:</b> €{((order?.totalAmount ?? 0) / 100).toFixed(2)}</div>
              {order?.paymentDueAt ? <div><b>Due:</b> {new Date(order.paymentDueAt).toLocaleString()}</div> : null}
            </div>
          </>
        ) : (
          <>
            ...
            <div className="bg-slate-50 border rounded p-3 text-sm">
              <div><b>Remaining Amount:</b> €{((order?.remainingAmount ?? 0) / 100).toFixed(2)}</div>
            </div>
          </>
        )}
      </div>
